...
class QueueController {
    ...
    async getAllQueues(req, res) {
        try {
            const queues = await QueueModel.find().sort({ queueNumber: 1 });
            res.status(200).json({
                success: true,
                data: queues
            });
    ...
    async getQueueStats(req, res) {
        try {
            ...
            const waitingQueues = await QueueModel.countDocuments({ status: 'waiting' });
            const inProgressQueues = await QueueModel.countDocuments({ status: 'in-progress' });
            const completedQueues = await QueueModel.countDocuments({ status: 'completed' });
            const cancelledQueues = await QueueModel.countDocuments({ status: 'cancelled' });
    ...
    async getNextQueue(req, res) {
        try {
            const nextQueue = await QueueModel.findOne({ status: 'waiting' }).sort({ queueNumber: 1 });
