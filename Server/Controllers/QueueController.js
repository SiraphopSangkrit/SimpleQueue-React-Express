...
class QueueController {
  ...
  async getQueuesByMonthSplitByWeek(req, res) {
    try {
      ...
      const weeksArray = Object.values(weeklyData).sort((a, b) => a.week - b.week);
      ...
      res.status(200).json({
        ...
        data: {
          ...
          summary: {
            ...
            averageQueuesPerWeek: Math.round(queues.length / weeksArray.length * 100) / 100
          }
        }
      });
    } catch (error) {
  ...
  // Alternative method with simpler week calculation (ISO week numbers)
  async getQueuesByMonthWithISOWeeks(req, res) {
    try {
      ...
      queues.forEach(queue => {
        ...
        const weekNumber = Math.ceil((firstThursday - tempDate) / 604800000) + 1;
      ...
      const weeksArray = Object.values(weeklyData).sort((a, b) => a.weekNumber - b.weekNumber);
